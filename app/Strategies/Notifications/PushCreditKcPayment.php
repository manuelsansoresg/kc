...
class PushCreditKcPayment implements SendNotificationsInterface
{
    public function send($id)
    {
        $data_notification = array(
            'id_rel' => $id,
            'title' => 'Crédito',
            'body' => 'Nuevo crédito en KC-Payments & KC-Aftermarket',
            'model' => HistoryLog::KC_PAYMENT,
        );
        ...
        if ($is_exist == 0) {
            ...
            $push  = new Pusher;
            $push->send(['model' => 'pushCreditKcPayment']);
        }
    }
    
    public function get($user_id = null, $status = 0)
    {
        $get_notifications = Notification::getByModel([HistoryLog::KC_PAYMENT]);
        $notifications = array();
