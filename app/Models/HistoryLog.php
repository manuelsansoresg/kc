...
class HistoryLog extends Model
{
    ...
    public function subHistories($id_rel, $status_id, $history)
    {
        ...
        if ($status_id == HistoryLog::SOLICITUD) {
            ...
            $getUsers = User::whereIn('id', $user_ids)->role('Cliente inversionista')
                ->where(function($query) {
                    $query->permission('Otorgar Vo.Bo')
                          ->orWhere(function($subQuery) {
                              $subQuery->permission('Administración');
                          });
                })->get();
        ...
        if ($status_id == HistoryLog::KC_AFTER_MARKET) {
            ...
            if ($manychat_id != null) {
                ...
                $data = array(
                    'URL Encuesta' => env('APP_URL') . '/survey/' . $credit->id,
                );
