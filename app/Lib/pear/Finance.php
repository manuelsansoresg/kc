...
class Finance
{
    ...
    */
    function presentValue($rate, $nper, $pmt, $fv = 0, $type = 0)
    {
        ...
        return self::roundDown($pv);
    }

    private function roundDown($value)
    {
        //* 26384.496023901
        $find_decimal             = strpos($value, '.');
        $string_before_decimal    = substr($value, 0, $find_decimal-3);
        $string_after_decimal    = substr($value, $find_decimal);
        $zero_right               = str_pad($string_before_decimal, $find_decimal, "0", STR_PAD_RIGHT);
        $value = $zero_right.$string_after_decimal;
        $result = round($value, -3 , PHP_ROUND_HALF_DOWN);
        return $result;
    }
